...
def get_transition_and_observation_f(model):
    ...
    """
    def lorenz_transition_function(state, noise):
        ...
        yv = r * x - y - x * z
        zv = x * y - b * z
        return np.array([xv, yv, zv]) * dt + state + noise

    def lorenz_observation_function(state, noise):
        C = np.array([[1, 0, 0], [0, 0, 0], [0, 0, 0]])
        return np.dot(C, state) + noise

    def vanderpol_transition_function(state, noise):
        mu = 2
        dt = 0.02
        x, y = state[0], state[1]
        xdt = mu * (x - 1/3 * x**3 - y)
    ...
    def vanderpol_observation_function(state, noise):
        C = np.array([[1, 0], [0, 0]])
        return np.dot(C, state) + noise
    
    if model == 'lorenz':
        return lorenz_transition_function, lorenz_observation_function
    if model == 'vanderpol':
    ...
    for training_idx in tqdm(range(start, end, 1)):
        ...
        if training_idx == start:
            ...
            for idx, (X_noisy_in, U_noisy_in) in enumerate(zip(X_noisy_in_list, U_noisy_in_list)): 
                ...
                # Initialize UKF
                if state_configuration == 'lorenz':
                    if training_name == 'lorenz_a':
                        transition_covariance = np.diag([0.3] * 3)
                        observation_covariance = np.diag([1.5] * 3)
                        initial_state_covariance = np.diag([1.5] * 3)
                    if training_name == 'lorenz_b':
                        transition_covariance = np.diag([0.2] * 3)
                        observation_covariance = np.diag([2] * 3)
                        initial_state_covariance = np.diag([2] * 3)
                    if training_name == 'lorenz_c':
                        transition_covariance = np.diag([0.1] * 3)
                        observation_covariance = np.diag([2.5] * 3)
                        initial_state_covariance = np.diag([2.5] * 3)
                ...
                X_unscented_pred[idx] = kf.filter(X_noisy_in)[0]
