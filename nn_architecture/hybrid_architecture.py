...
from .pytorch_system import vanderpol, lorenz, coupled_mass
...
class Hybrid_resnet(Nn_base):
    ...
    def __init__(
        ...
        if prior_model == 'lorenz':
            self.prior_model = lorenz()
        ...
        if prior_model == 'coupled_mass':
            self.prior_model = coupled_mass()
            self.offset = torch.ones((1, 1, len(self.prior_model.state_variables))).to('cuda')
            self.offset[:, :, 0] = 0.
            self.offset[:, :, 1] = 0.
            self.offset[:, :, 2] = 0.
        ...
        self.model = Mlp(
            input_size = sequence_size, # // 2**1 + 1) * 1,
        ...
        if mode == 'forward_prediction':
            ...
            for i in range(self.nb_integration_steps):
                ...
                for _ in range(50):
                    ...
                    # k1 = self.op(X_out, U_out[:, i].unsqueeze(1)).unsqueeze(1)
                    # k2 = self.op(X_out + dt/2 * k1, U_out[:, i].unsqueeze(1)).unsqueeze(1)
                    # k3 = self.op(X_out + dt/2 * k2, U_out[:, i].unsqueeze(1)).unsqueeze(1)
                    # k4 = self.op(X_out + dt * k3, U_out[:, i].unsqueeze(1)).unsqueeze(1)
                    # X_out = X_out + (k1 + 2 * k2 + 2 * k3 + k4) * dt * 1/6.
